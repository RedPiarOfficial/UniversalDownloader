...
class JutSu:
	...
	def download_chunk(self, url, chunk_size, start_byte, end_byte, filename, progress):
		with httpx.Client(http2=True) as client:
			...
			# Используем httpx.stream() для загрузки данных в потоковом режиме
			with client.stream("GET", url, headers=headers) as resp:
				resp.raise_for_status()
				# Вызов метода read() для получения данных из потока
				chunk_data = resp.read()  # Читаем данные из потока
	...
	def download_anime_episode(self, DownloadLink, DownloadID=None, link=None):
		...
		# Получаем размер файла без использования контекстного менеджера
		with httpx.Client(http2=True) as client:
			resp = client.head(DownloadLink, headers=headers)
			resp.raise_for_status()  # Убедимся, что запрос прошел успешно
		...
		with tqdm(total=total_size, unit='B', unit_scale=True, unit_divisor=1024, desc=f"Сезон {season}, Эпизод {episode.group(1)}") as progress:
			...
			# Создаем пул потоков для скачивания чанков
